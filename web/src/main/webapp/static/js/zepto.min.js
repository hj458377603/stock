...
var Queuejs = function() {
    var d = document,
        ...
        file = [];
    //主要方法,根据文件数量执行次数
    function ready(_source, callback) {
        var appendDom, 
            url;

        if (_source.src) {
            //检测参数是否是数组
            _source.src = _source.src.constructor === Array ? _source.src : [_source.src];
            //获取所有需要异步载入的脚本文件，拆分数据
            for (var i = 0; i < _source.src.length; i++) {
                if(_source.src[i].constructor !== Array){
                    //src变成数组变量
                    _source.src[i] = [_source.src[i]];
                }
                file.push(_source.src[i]);
            }
        }
        //现取出第一个file[0],出队列
        tmpPara = file.shift();
        headTag = headTag || d.getElementsByTagName("head")[0];
        //第一个urls,重新赋值
        _source.src = tmpPara;
        //添加文件
        for (var i = 0; i < _source.src.length; i++) {
            url = _source.src[i];
            appendDom = d.createElement("script");
            appendDom.src = url;
            appendDom.type = "text/javascript";
            //加载完毕和出错都载入下一个文件，防止资源阻塞
            appendDom.onload = appendDom.onerror = function() {
                load(_source, callback);
            };
            headTag.appendChild(appendDom);
        }
    };
    //载入资源文件
    function load(_source, callback) {
        //当前资源文件
        var p = tmpPara;
        if (!p) {
            return;
...
/**
 * [description]
 * @param  {[Array]} srcArr             [加载的 js ]
 * @param  {[Function]} loadComplete   [加载完毕的回调]
 * @param  {[Boolean]} withoutGa      [是否需要埋点 js]
 * @return {[type]}                [description]
 */
var _loadScript_ = (function(options){
    var loadComplete = options.loadComplete || function(){};
    var withoutGa = options.withoutGa || false;
    ...
    var loadObj = {
        src: options.srcArr
    },LoadCallbackFn = loadComplete;

    // 根据目前首页业务需求控制加载顺序
    //
    function LoadJSFunction(){
    	_loadScript_.prototype.uuid++;
        // 自动构建wap 流量埋点, 判断是否客户端
        if(_loadScript_.prototype.uuid <= 1){
        ...
        // 最后加载业务数据，跟页面无关js
        if(isDelay && isCache && !withoutGa && _loadScript_.prototype.uuid <= 1){
            /*
                A方式下onload完毕最后加载  
             */
            
            Queuejs.init({
                src: "${script.domain}/javascript/sn_da/da_opt.js"
            });
            return;
        }
        isDelay = true;
        if(isCache){
            /*
                此时为A方式，和未onload资源同步加载
                
             */
            Queuejs.init(loadObj,LoadCallbackFn);
        }else{
            /*
                两秒内加载完毕，B方式
             */
            if(!withoutGa && _loadScript_.prototype.uuid <= 1){
                loadObj.src.push("${script.domain}/javascript/sn_da/da_opt.js");
            }
            Queuejs.init(loadObj,LoadCallbackFn);
        }
        
    }
    // 两秒内加载到此方法且页面没有onload完毕，执行。假设此时为A方式
    setTimeout(function(){
        isCache = true;
        // 先B方式然后执行，则return；
        if(!isDelay){
    ...
    // 因网速很快，或者有缓存2秒页面即加载完毕则直接执行。假设此时为B方式
    // A方式下onload完毕，此时isCache = true, isDelay = true; 
...
/* 
* @Author: 12050231
* @Date:   2014-09-01 16:32:28
* @Last Modified by:   12050231
* @Last Modified time: 2015-05-26 18:22:48
*/
